...
{
    public const RESTRICTION_TYPE = 'Master';
     ...
     */
    public function __construct(BookingEngineInterface $bookingEngine, ProductLoader $productLoader, PartnerLoader $partnerLoader, AuthorizationCheckerInterface $authorizationChecker, ProductAvailabilityCollectionFactory $availabilityCollectionFactory, AvailabilityFactory $availabilityFactory, CmhubLogger $logger)
    {
    ...
    public function handle(\StdClass $request): array
    {
        ...
        $productAvailabilityCollection = $this->availabilityCollectionFactory->create($partner);
        ...
        foreach ($inventories as $inventory) {
            ...
            $availability = $this->availabilityFactory->create($startDate, $endDate, null, $product);

            if (isset($inventory->BookingLimit)) {
                $availability->setStock($inventory->BookingLimit);
            }

            if (isset($inventory->RestrictionStatus, $inventory->RestrictionStatus->Status, $inventory->RestrictionStatus->Restriction) &&
                self::RESTRICTION_TYPE === $inventory->RestrictionStatus->Restriction
            ) {
                if (RestrictionStatus::CLOSE === $inventory->RestrictionStatus->Status) {
                    $availability->setStopSale(true);
                }

                if (RestrictionStatus::OPEN === $inventory->RestrictionStatus->Status) {
